...
class Achievement extends EventProvider implements ServiceManagerAwareInterface
{
    ...
    public function getBadges($user)
    {
        ...
        $query = $em->createQuery("SELECT MAX(a.level) as level, a.category, a.levelLabel, a.label FROM PlaygroundReward\Entity\Achievement a WHERE a.user = :user AND a.type = 'badge' GROUP BY a.category");
        $query->setParameter('user', $user);
        $result = $query->getResult();
